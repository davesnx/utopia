/* Utopia_dev_client.re — Browser dev client runtime
   ...
   This library is only included in development builds:
   Generated_dune.ml conditionally adds it to the melange.emit
   libraries list, and esbuild.config.mjs bundles the compiled
   output as a separate entry point. */
...
function _ensureProgressBarStyles() {
  if (document.getElementById('utopia-dev-progress-styles')) return;
  var style = document.createElement('style');
  style.id = 'utopia-dev-progress-styles';
  style.textContent = '@keyframes utopia-progress{0%{width:0%}20%{width:30%}50%{width:60%}80%{width:85%}100%{width:95%}}';
...
function _renderOverlay() {
  ...
  var hasErrors = hasBuildErrors || hasBuildWarnings || hasRuntimeErrors;

  if (!hasErrors && !isRebuilding && !isReconnecting) {
    root.innerHTML = '';
    root.style.pointerEvents = 'none';
    return;
  }

  // --- Progress bar for rebuilding / reconnecting ---
  var html = '';
  if (isRebuilding || isReconnecting) {
    _ensureProgressBarStyles();
    var barColor = isReconnecting ? '#a0a0ff' : '#6366f1';
    html += '<div style="position:fixed;top:0;left:0;width:100%;height:3px;z-index:1000000;background:rgba(0,0,0,0.1);">';
    html += '<div style="height:100%;background:' + barColor + ';animation:utopia-progress 8s ease-out forwards;will-change:width;"></div>';
    html += '</div>';
  }

  // --- Error/warning modal (only when there are actual problems) ---
  if (hasErrors) {
    root.style.pointerEvents = 'auto';
    html += '<div style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.75);display:flex;justify-content:center;align-items:flex-start;padding-top:40px;overflow-y:auto;">';
  ...
  source.onmessage = function(event) {
    try {
      ...
      if (state && state.kind === 'build_state') {
        var prev = window.__utopia_dev_build_state;
        window.__utopia_dev_build_state = state;
        window.__utopia_dev_reconnecting = false;
        _renderOverlay();

        // Build recovered to healthy — reload to pick up changes.
        // (If the server also restarted, the onopen handler may
        //  fire a redundant reload which is harmless.)
        if (state.status === 'healthy' && !state.rebuilding &&
            prev && prev.status !== 'healthy') {
...
// (esbuild bundles this as an ES module, scoping all declarations).
window._renderOverlay = _renderOverlay;
