...
`Utopia.useRouter()` returns the current request path, the current `Utopia.Route.t`, a generated `current` match value (`option(Utopia.Routes.Current.t)`), and a `navigate(~replace=?, ~revalidate=?, route)` function that accepts typed route values for SPA-style navigation.
