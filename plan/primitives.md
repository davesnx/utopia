...
### Route Manifest
A TSV file at `_utopia/routes.manifest` generated by the compiler. Each row contains: route, kind, source_file, matcher, params, layouts, has_metadata, and static. The server reads this at startup to build its routing table, and generated-server builds also use the same route metadata when constructing compiled route descriptors. See "Matcher Format" for the wire representation of route segments.
...
### Frontmatter
YAML metadata block at the top of a markdown page, delimited by `---`. Compiler-level markdown-page frontmatter is still planned; when it lands it will be parsed by the compiler, stripped before rendering, and support fields such as `title`, `description`, `path` (route override), and `layout` (explicit layout selection).
...
A static, runnable JavaScript file that imports `server-reason-react-esbuild-plugin`, configures entry points (the client entry), target directory (Melange output), and output directory. Executed by Node as a dune rule. The config is a static runtime file copied from `lib/utopia_runtime/files/esbuild.config.mjs` — not generated. It imports build metadata from a sibling `_utopia/paths.mjs` module, sets `process.env.NODE_ENV` internally for Node tooling, enables production minification when `buildMode = "production"`, and derives all build paths at runtime. This means the esbuild config is real, directly runnable JavaScript with full editor support, linting, and syntax highlighting.
...
The generated `_utopia/Utopia.re` file is now a tiny project-specific wrapper only. It `include`s the shared `Utopia_base` module from the public `utopia` library and attaches the generated `Utopia_routes` module as `Utopia.Routes`. The copied runtime support modules (`Utopia_call_server`, `Utopia_route`, `Utopia_router`, `Utopia_router_link`, `Utopia_router_route`, `Utopia_server`, and `Utopia_types`) are no longer generated into project trees; generated code depends on the shared `utopia` library instead.

### Generated Runtime Support Library (`utopia_runtime`)
An internal compiler-support library under `lib/utopia_runtime/` that ships the static source files copied into generated projects: `ReactServerDOMEsbuild.re`, `FunctionReferences.re`, `Utopia.re`, `Utopia_call_server.re`, `Utopia_route.ml`, `Utopia_route_builder.ml`, `esbuild.config.mjs`, `Utopia_server.ml`, `Utopia_types.ml`, `Utopia_router.re`, `Utopia_router_route.re`, `Utopia_router_link.re`, and `client_entry.re`.
...
The framework's server logic, extracted as a library under `lib/server/` and also copied into generated projects as `_utopia/Utopia_server.ml`. Contains: Dream-based HTTP routing, route manifest loading, generated-route loading, asset serving, streamed RSC/HTML/action response helpers, cache management, and request handling logic. Does NOT contain page-specific user route code.

### Server Executable (per-project)
A generated executable at `_utopia/server_main.ml` that wires compiler-generated route descriptors into the copied `Utopia_server` support module. It links a generated project-scoped native pages library (for example `pages_demo_notes`) so multiple Utopia projects can coexist in the same Dune workspace without library-name collisions. Generated by the compiler for each project.
...
### Live Reload
Planned dev-mode browser behavior. The intended first implementation is full reload only: no HMR, no React state preservation, and no per-module hot swapping. Current `utopia dev` restarts the generated server and streams dune RPC diagnostics to the terminal only; it does not yet notify the browser directly.

### Server Restart
In dev mode, the CLI restarts the generated per-project server executable after successful rebuilds. It prefers the requested port but currently re-probes for the next available port before each start/restart, so the origin can move if the old port becomes unavailable.

### Dev Event Channel
Planned development-only server channel for browser build-state observation. The intended design is an SSE subscription endpoint plus a CLI-authenticated publish endpoint, but this is not implemented in the current CLI/server/runtime path.
...
### Static Page
A page that exports `let static = true`. The compiler detects this via source text scanning and records the flag in the route manifest. Static pages are rendered at build time by the `--ssg` mode of the generated `server_main.exe`, producing plain HTML files in `_utopia/static/`.
